...
import { getReserveInfo } from "shared/actions/reserveInfo";
...
export const addDesktopStoreWatchers = (
  store: Store<DesktopAppState, Action<DesktopAppState>> & { dispatch: DispatchFunctionType },
) => {
  ...
  store.subscribe(
    connectWatcher((isConnected: boolean, wasConnected: boolean) => {
      if (isConnected && !wasConnected) {
        store.dispatch(initChainData());
        store.dispatch(getLastBlockHeader());
        store.dispatch(getReserveInfo());
  ...
  store.subscribe(
    syncWatcher((isSyncedNow: boolean, wasSyncedBefore: boolean) => {
