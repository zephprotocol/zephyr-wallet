...
import { GET_TRANSFERS_SUCCEED } from "../actions/types";
...
import MoneroIncomingTransfer from "zephyr-javascript/src/main/js/wallet/model/MoneroIncomingTransfer";
import { TxEntry, Conversion } from "shared/components/tx-history/container";
import { bigIntegerToBigInt } from "utility/utility";
import MoneroOutgoingTransfer from "zephyr-javascript/src/main/js/wallet/model/MoneroOutgoingTransfer";

const INITAL_STATE: MoneroTxWallet[] = [];

export const xTransferList = (state = INITAL_STATE, action: AnyAction): MoneroTxWallet[] => {
  switch (action.type) {
...
export const selectTransferListByTicker = (state: ZephyrAppState, tickerId: Ticker): TxEntry[] => {
  const walletTxs = state.xTransferList;
  const txEntries: TxEntry[] = walletTxs.reduce((list: TxEntry[], walletTx: MoneroTxWallet) => {
    ...
    if (incomings !== undefined && incomings[0].getCurrency() === tickerId) {
      ...
      txEntry.timestamp = walletTx.isConfirmed() ? walletTx.getBlock().getTimestamp() : Date.now() / 1000;
  ...
  txEntries.sort((a: TxEntry, b: TxEntry) =>
    b.isFailed !== a.isFailed ? (b.isFailed ? -1 : 1) : b.timestamp - a.timestamp,
  );
...
function getConversion(walletTx: MoneroTxWallet, isIncoming: boolean): Conversion {
  ...
  let possibleConversion: MoneroOutgoingTransfer = isIncoming
    ? walletTx.getOutgoingTransfer()
    : walletTx.getIncomingTransfers();
...
export const hasNoTxsEntries = (state: ZephyrAppState): boolean => {
  return state.xTransferList.length === 0;
};

export const getHeightOfFirstIncomingTx = (state: ZephyrAppState): number => {
  const currentHeight = state.chain.walletHeight;
  const walletTxs = state.xTransferList;

  const firstTxHeight: number = walletTxs.reduce((height: number, walletTx: MoneroTxWallet) => {
    const incomings: undefined | MoneroIncomingTransfer[] = walletTx.getIncomingTransfers();
