import { GET_BLOCK_INFO_SUCEED, GET_WALLET_HEIGHT_SUCCEED } from "shared/actions/types";
import { AnyAction } from "redux";
import { SyncState } from "shared/types/types";
import { DesktopAppState, ZephyrAppState } from "platforms/desktop/reducers/index";
...
export const selectBlockHeight = (state: ZephyrAppState) => {
  return state.chain.chainHeight;
};

export const selectNodeHeight = (state: ZephyrAppState) => {
  return state.chain.nodeHeight;
};

export const selectSyncState = (state: ZephyrAppState): SyncState => {
...
export const selectWalletHeight = (state: ZephyrAppState) => {
  return state.chain.walletHeight;
};

export const isWalletSynced = (state: ZephyrAppState): boolean => {
