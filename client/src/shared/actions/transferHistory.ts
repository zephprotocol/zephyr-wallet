  ...
  return async (dispatch: any) => {
    ...
    try {
      ...
      if (transfers && transfers.length > 0) {
        let dt = new Date();
        let datestr =
          dt.toISOString().split("T")[0] + "_" + dt.toISOString().split("T")[1].split(".")[0].replace(":", "");
        let str_out = "";
        let filename = "zephyr_tx-" + datestr;
          ...
          for (let i = 0; i < transfers.length; i++) {
            ...
            if (tx_csv_obj.length > 0) {
              ...
              for (let r = 0; r < tx_csv_obj.length; r++) {
                ...
                tx_csv_obj[r]["Fee"] =
                  tx_csv_obj[r]["Fee"] > 0 ? convertBalanceToMoney(tx_csv_obj[r]["Fee"], numDecimals) : 0;
                tx_csv_obj[r]["Amount"] =
                  tx_csv_obj[r]["Amount"] > 0 ? convertBalanceToMoney(tx_csv_obj[r]["Amount"], numDecimals) : 0;
