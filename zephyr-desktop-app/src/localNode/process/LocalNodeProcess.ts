...
export class LocalNodeProcess {
  ...
  public onstderrData(chunk: any): void {
    if (isDevMode) {
      console.error("zephyrd stderr : " + chunk.toString());
    }
  }

  public onstdoutData(chunk: any): void {
    if (isDevMode) {
      console.error("zephyrd stdout : " + chunk.toString());
    }
  }

  public killDaemon(): void {
    if (isDevMode) {
      console.log(`try to kill zephyrd`);
    }
  ...
  public startLocalProcess(): void {
    ...
    const args: ReadonlyArray<string> = Object.entries(this.startArgs).map(([key, value]) => {
  ...
  private onDaemonExit(code: number | null, signal: string | null): void {
    ...
    if (code !== 0 && code !== null) {
      dialog.showErrorBox(`zephyrd not running`, "Process was stopped or did not even start");
    }

    if (isDevMode) {
      console.log(`zephyrd exit with code ${code} and signal ${signal}`);
    }
  }
